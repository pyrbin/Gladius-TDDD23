...
#   rename item_container controllers/items to containers


const Container = preload("item_container.gd")
...
func connect_controller(container):
    if not _connected_container_controllers.has(container):
        _connected_container_controllers.append(container)

func disconnect_controller(container):
    var idx = _connected_container_controllers.find(container)
    if idx != -1:
        _connected_container_controllers.remove(idx)

func connect_to_item_container(p_item_container):
    if item_container:
...
func _update_item_slot(slot):
    ...
    if item == null:
        _set_empty(slot)
        return
    container_list.set_item_icon(slot, load(item.icon))
