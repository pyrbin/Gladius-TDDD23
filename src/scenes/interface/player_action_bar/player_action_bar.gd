...
func _process(delta):
    if status:
        life_text.set_text(String(status.health) + " of " + String(status.get_max_health()))
        life_bar.value = (status.health/status.get_max_health())
        energy_bar.value = float(status.endurance)/status.get_max_endurance()
        energy_text.set_text(String(status.endurance) + " of " + String(status.get_max_endurance()))
